...
MINIO_MIRROR_MAX_RETRY=${MINIO_MIRROR_MAX_RETRY:-3}
...
if [ "${COMMAND}" = "backup" ] ; then
  ...
  do
    ...
    while true;
    do
      "${MC}" "${MC_OPTS[@]}" mirror "${MC_MIRROR_OPTS[@]}" "${EXTRA_MC_MIRROR_COMMAND[@]}" wdminio/${bucket} ${MINIO_BACKUP_DIR}/${bucket} 2>&1
      RC=$?
      echo "RC=${RC}"
      if [ $RC -eq 0 ] ; then
        break
      ...
      if [ ${retry_count} -ge ${MINIO_MIRROR_MAX_RETRY} ] ; then
        brlog "WARN" "Could not fully mirror bucket ${bucket} after ${MINIO_MIRROR_MAX_RETRY} retries (RC=${RC}). Skipping remaining objects and continuing."
        break
      fi
      brlog "WARN" "Some file could not be transfered from bucket ${bucket} (RC=${RC}). Retrying... (${retry_count}/${MINIO_MIRROR_MAX_RETRY})"
    done
    set -e
    cd - > /dev/null
  done
  ...
  do
    ...
    if "${MC}" "${MC_OPTS[@]}" ls wdminio | grep ${bucket} > /dev/null ; then
      ...
      do
        ...
        retry_count=$((retry_count + 1))
        if [ ${retry_count} -ge ${MINIO_MIRROR_MAX_RETRY} ] ; then
          brlog "WARN" "Could not fully restore bucket ${bucket} after ${MINIO_MIRROR_MAX_RETRY} retries (RC=${RC}). Skipping remaining objects and continuing."
          break
        fi
        brlog "WARN" "Some file could not be transfered from bucket ${bucket} (RC=${RC}). Retrying... (${retry_count}/${MINIO_MIRROR_MAX_RETRY})"
      done
