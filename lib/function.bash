...
is_component_skipped(){
  local name="$1"
  [ -z "${SKIP_COMPONENTS:-}" ] && return 1
  local c
  for c in ${SKIP_COMPONENTS}; do
    if [ "$c" = "$name" ] ; then
      return 0
    fi
  done
...
filter_skipped_components(){
  local result=()
  local c
  for c in "$@"; do
    if ! is_component_skipped "$c" ; then
      result+=("$c")
    fi
  done
  echo "${result[@]}"
...
check_datastore_available(){
  ...
  if is_component_skipped etcd ; then
    brlog "WARN" "Skip checking Etcd (--skip-components)"
  else
    check_etcd_available || { brlog "ERROR" "Etcd is unavailable"; return 1; }
  fi
  if is_component_skipped postgresql ; then
    brlog "WARN" "Skip checking Postgresql (--skip-components)"
  else
    check_postgres_available || { brlog "ERROR" "Postgresql is unavailable"; return 1; }
  fi
  if is_component_skipped elastic ; then
    brlog "WARN" "Skip checking ElasticSearch (--skip-components)"
  else
    check_elastic_available || { brlog "ERROR" "ElasticSearch is unavailable"; return 1; }
  fi
  if is_component_skipped minio ; then
    brlog "WARN" "Skip checking S3 (--skip-components)"
  else
