...
if ! "${ELASTIC_SCHEMA_ONLY}" && [ $(compare_version ${WD_VERSION} "4.7.0") -ge 0 ] && [ $(compare_version ${WD_VERSION} "5.2.0") -lt 0 ]; then
  mount_pvc_to_elasitc
fi

if ! "${ELASTIC_SCHEMA_ONLY}" && [ $(compare_version ${WD_VERSION} "5.2.0") -ge 0 ]; then
...
###############
# Schema-only mode: backup/restore index mappings and settings without snapshot data.
# Documents become searchable again after re-ingesting content from MinIO/sources.
###############
if "${ELASTIC_SCHEMA_ONLY}" ; then
  # Build the in-pod ELASTIC_ENDPOINT prefix; for WD<5.2.0 the pod has no env var.
  if [ $(compare_version ${WD_VERSION} "5.2.0") -lt 0 ]; then
    _es_env='export ELASTIC_ENDPOINT=https://localhost:9200'
  else
    _es_env='true'
  fi

  if [ "${COMMAND}" = 'backup' ] ; then
    brlog "INFO" "Start schema-only backup of ElasticSearch (mappings + settings, no document data)"
    mkdir -p "${TMP_WORK_DIR}/${ELASTIC_BACKUP_DIR}"

    brlog "INFO" "Fetching index mappings from ElasticSearch pod..."
    run_cmd_in_pod ${ELASTIC_POD} \
      "${_es_env} && curl -s -k -u \${ELASTIC_USER}:\${ELASTIC_PASSWORD} \"\${ELASTIC_ENDPOINT}/_all/_mappings\" > /tmp/es_schema_mappings.json" \
      ${OC_ARGS} -c "${ELASTIC_POD_CONTAINER}"

    brlog "INFO" "Fetching index settings from ElasticSearch pod..."
    run_cmd_in_pod ${ELASTIC_POD} \
      "${_es_env} && curl -s -k -u \${ELASTIC_USER}:\${ELASTIC_PASSWORD} \"\${ELASTIC_ENDPOINT}/_all/_settings\" > /tmp/es_schema_settings.json" \
      ${OC_ARGS} -c "${ELASTIC_POD_CONTAINER}"

    kube_cp_to_local ${ELASTIC_POD} "${TMP_WORK_DIR}/${ELASTIC_BACKUP_DIR}/mappings.json" "/tmp/es_schema_mappings.json" ${OC_ARGS} -c "${ELASTIC_POD_CONTAINER}"
    kube_cp_to_local ${ELASTIC_POD} "${TMP_WORK_DIR}/${ELASTIC_BACKUP_DIR}/settings.json" "/tmp/es_schema_settings.json" ${OC_ARGS} -c "${ELASTIC_POD_CONTAINER}"
    echo "schema_only" > "${TMP_WORK_DIR}/${ELASTIC_BACKUP_DIR}/backup_type"

    brlog "INFO" "Archiving schema backup as ${BACKUP_FILE}"
    tar "${ELASTIC_TAR_OPTIONS[@]}" -cf "${BACKUP_FILE}" -C "${TMP_WORK_DIR}/${ELASTIC_BACKUP_DIR}" .
    if "${VERIFY_DATASTORE_ARCHIVE}" && brlog "INFO" "Verifying backup archive" && ! tar "${ELASTIC_TAR_OPTIONS[@]}" -tf "${BACKUP_FILE}" &>/dev/null ; then
      brlog "ERROR" "Backup file is broken, or does not exist."
      exit 1
    fi
    brlog "INFO" "Schema-only ElasticSearch backup completed: ${BACKUP_FILE}"
    echo
    rm -rf "${TMP_WORK_DIR}"
    if [ -z "$(ls tmp)" ] ; then rm -rf tmp; fi
    exit 0
  fi

  if [ "${COMMAND}" = 'restore' ] ; then
    if [ -z "${BACKUP_FILE}" ] ; then printUsage; fi
    ...
    brlog "INFO" "Start schema-only restore of ElasticSearch (creating empty indices from saved schema)"
    mkdir -p "${TMP_WORK_DIR}/${ELASTIC_BACKUP_DIR}"
    tar "${ELASTIC_TAR_OPTIONS[@]}" -xf "${BACKUP_FILE}" -C "${TMP_WORK_DIR}/${ELASTIC_BACKUP_DIR}"

    if [ ! -f "${TMP_WORK_DIR}/${ELASTIC_BACKUP_DIR}/backup_type" ] || \
       [ "$(cat "${TMP_WORK_DIR}/${ELASTIC_BACKUP_DIR}/backup_type")" != "schema_only" ] ; then
      brlog "ERROR" "Backup file is not a schema-only backup. Use regular restore for full snapshot backups."
      exit 1
    fi

    # Write the in-pod restore script.
    cat > "${TMP_WORK_DIR}/schema_restore.sh" << 'EOSCRIPT'
#!/bin/bash
set -euo pipefail
...
curl -XDELETE -s -k -u "${ELASTIC_USER}:${ELASTIC_PASSWORD}" \
  "${ELASTIC_ENDPOINT}/_all?master_timeout=30m" || true
curl -XDELETE -s -k -u "${ELASTIC_USER}:${ELASTIC_PASSWORD}" \
  "${ELASTIC_ENDPOINT}/.*?master_timeout=30m" || true

echo "Recreating indices from saved schema..."
while IFS= read -r index; do
  if [[ "${index}" == application_logs-* ]]; then
    echo "  Skipping ${index}"
    continue
  fi
  settings=$(jq --arg i "${index}" '.[$i].settings.index | del(.creation_date, .uuid, .version, .provided_name)' "${SETTINGS_FILE}")
  mappings=$(jq --arg i "${index}" '.[$i].mappings' "${MAPPINGS_FILE}")
  body=$(jq -n --argjson s "${settings}" --argjson m "${mappings}" '{"settings":{"index":$s},"mappings":$m}')
  result=$(curl -XPUT -s -k -u "${ELASTIC_USER}:${ELASTIC_PASSWORD}" \
    "${ELASTIC_ENDPOINT}/${index}?master_timeout=30m" \
    -H "Content-Type: application/json" -d "${body}")
  if echo "${result}" | grep -q '"acknowledged":true'; then
    echo "  Created: ${index}"
  else
    echo "  WARN: ${index}: ${result}"
  fi
done < <(jq -r 'keys[]' "${MAPPINGS_FILE}")
echo "Schema restore complete"
EOSCRIPT
    chmod +x "${TMP_WORK_DIR}/schema_restore.sh"

    brlog "INFO" "Copying schema files to ElasticSearch pod..."
    kube_cp_from_local ${ELASTIC_POD} "${TMP_WORK_DIR}/schema_restore.sh"             "/tmp/es_schema_restore.sh"     ${OC_ARGS} -c "${ELASTIC_POD_CONTAINER}"
    kube_cp_from_local ${ELASTIC_POD} "${TMP_WORK_DIR}/${ELASTIC_BACKUP_DIR}/mappings.json" "/tmp/es_schema_mappings.json"  ${OC_ARGS} -c "${ELASTIC_POD_CONTAINER}"
    kube_cp_from_local ${ELASTIC_POD} "${TMP_WORK_DIR}/${ELASTIC_BACKUP_DIR}/settings.json" "/tmp/es_schema_settings.json"  ${OC_ARGS} -c "${ELASTIC_POD_CONTAINER}"

    brlog "INFO" "Running schema restore in ElasticSearch pod..."
    run_cmd_in_pod ${ELASTIC_POD} \
      "${_es_env} && bash /tmp/es_schema_restore.sh /tmp/es_schema_mappings.json /tmp/es_schema_settings.json" \
      ${OC_ARGS} -c "${ELASTIC_POD_CONTAINER}"
    brlog "INFO" "$(get_last_cmd_result_in_pod)"

    run_cmd_in_pod ${ELASTIC_POD} \
      "rm -f /tmp/es_schema_restore.sh /tmp/es_schema_mappings.json /tmp/es_schema_settings.json" \
      ${OC_ARGS} -c "${ELASTIC_POD_CONTAINER}"
    ...
    brlog "INFO" "Schema-only ElasticSearch restore completed"
    echo
    rm -rf "${TMP_WORK_DIR}"
    if [ -z "$(ls tmp)" ] ; then rm -rf tmp; fi
    exit 0
