...
Options:
    ...
    --skip-components <comp1,comp2,...>        Skip specified components during backup/restore. Values: wddata, etcd, postgresql, elastic, minio.
                                               WARNING: the resulting backup is INCOMPLETE. You must recover the skipped component's state yourself
                                               (for example, reindex ElasticSearch from PostgreSQL and MinIO after restore).
...
do
  ...
  case $OPT in
    ...
    --skip-components)
      ...
      export SKIP_COMPONENTS="$(echo "$2" | tr ',' ' ')"
      shift 1
      ;;
    --skip-components=*)
      export SKIP_COMPONENTS="$(echo "${1#--skip-components=}" | tr ',' ' ')"
      ;;
    -- | -)
      shift 1
      param+=( "$@" )
...
if [ -n "${SKIP_COMPONENTS:-}" ] ; then
  VALID_SKIP_COMPONENTS="wddata etcd hdp postgresql elastic minio"
  for skip in ${SKIP_COMPONENTS}; do
    if ! echo " ${VALID_SKIP_COMPONENTS} " | grep -q " ${skip} " ; then
      brlog "ERROR" "Invalid component name in --skip-components: '${skip}'. Valid values: ${VALID_SKIP_COMPONENTS}"
      exit 1
    fi
  done
  brlog "WARN" "Skipping components: ${SKIP_COMPONENTS}. The backup/restore will be INCOMPLETE for these components."
fi
...
if [ ${COMMAND} = 'backup' ] ; then
  if [ $(compare_version "${WD_VERSION}" "5.3.0") -lt 0 ] && ! is_component_skipped elastic ; then
  ...
  if [[ ${REINDEX_OLD_INDEX} = "true" ]] && ! is_component_skipped elastic ; then
  ...
  if [ -n "${SKIP_COMPONENTS:-}" ] ; then
    ALL_COMPONENT=( $(filter_skipped_components "${ALL_COMPONENT[@]}") )
  fi
